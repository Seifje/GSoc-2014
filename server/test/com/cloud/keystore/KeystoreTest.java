...
public class KeystoreTest extends TestCase {
    ...
    /*
     * public void testKeystoreSave() throws Exception { KeystoreVO ksVo;
     *
     * ComponentLocator locator = ComponentLocator.getCurrentLocator();
     *
     * KeystoreDao ksDao = locator.getDao(KeystoreDao.class);
     * ksDao.save("CPVMCertificate", "CPVMCertificate", "KeyForCertificate",
     * "realhostip.com"); ksVo = ksDao.findByName("CPVMCertificate");
     ...
     *
     * ksVo = ksDao.findByName("CPVMCertificate"); assertTrue(ksVo != null);
     ...
     *
     * public void testStripeKey() throws Exception { Pattern regex =
     * Pattern.compile
     * ("(^[\\-]+[^\\-]+[\\-]+[\\n]?)([^\\-]+)([\\-]+[^\\-]+[\\-]+$)"); Matcher
     * m = regex.matcher(
     ...
     * ); if(m.find()) { String content = m.group(2);
     * assertTrue(content.startsWith("MIIEpAIBAAKCAQE"));
     * assertTrue(content.endsWith("KghyqAF1w==\n")); } else {
     * assertTrue(false); } }
     *
     * public void testKeystoreManager() throws Exception { ComponentLocator
     ...
     * Object>())); assertTrue(ksMgr.start());
     *
     * ksMgr.saveCertificate("CPVMCertificate", certContent, keyContent,
     * "realhostip.com");
     *
     * byte[] ksBits = ksMgr.getKeystoreBits("CPVMCertificate", "realhostip",
     * "vmops.com"); assertTrue(ksBits != null);
     *
     * try { KeyStore ks = CertificateHelper.loadKeystore(ksBits, "vmops.com");
     * assertTrue(ks != null); } catch(Exception e) { assertTrue(false); }
     *
     * KeystoreDao ksDao = locator.getDao(KeystoreDao.class); KeystoreVO ksVo =
     * ksDao.findByName("CPVMCertificate"); ksDao.expunge(ksVo.getId()); }
     */
    public void testUuid() {
        UserVmResponse vm = new UserVmResponse();
        vm.setId(Long.toString(3L));
        /*
         * vm.setAccountName("admin"); vm.setName("i-2-3-KY");
         * vm.setDisplayName("i-2-3-KY"); vm.setDomainId(1L);
         * vm.setDomainName("ROOT"); vm.setCreated(new Date());
         * vm.setState("Running"); vm.setZoneId(1L); vm.setZoneName("KY");
         * vm.setHostId(1L);
         *
