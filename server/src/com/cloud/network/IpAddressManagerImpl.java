...
public class IpAddressManagerImpl extends ManagerBase implements IpAddressManager, Configurable {
    ...
    // 1.there is at least one public IP associated with the network on which
    // first rule (PF/static NAT/LB) is being applied.
    // 2.last rule (PF/static NAT/LB) on the public IP has been revoked. So the
    // public IP should not be associated with any provider
    boolean checkIfIpAssocRequired(Network network, boolean postApplyRules, List<PublicIp> publicIps) {
        ...
        // there are no IP's corresponding to this network that need to be
    ...
    public boolean applyRules(List<? extends FirewallRule> rules, FirewallRule.Purpose purpose, NetworkRuleApplier applier, boolean continueOnError)
        ...
        // rules can not programmed unless IP is associated with network service
        // provider, so run IP assoication for
        // the network so as to ensure IP is associated before applying rules
        // (in add state)
        ...
        // if there are no active rules associated with a public IP, then public
        // IP need not be associated with a provider.
        // This IPAssoc ensures, public IP is dis-associated after last active
        // rule is revoked.
    ...
    protected boolean cleanupIpResources(long ipId, long userId, Account caller) {
        ...
        // remote access vpn can be enabled only for static nat ip, so this part
        // should never be executed under normal
        // conditions
        // only when ip address failed to be cleaned up as a part of account
        // destroy and was marked as Releasing, this part of
    ...
    public PublicIp fetchNewPublicIp(final long dcId, final Long podId, final List<Long> vlanDbIds, final Account owner, final VlanType vlanUse, final Long guestNetworkId,
        ...
        IPAddressVO addr = Transaction.execute(new TransactionCallbackWithException<IPAddressVO, InsufficientAddressCapacityException>() {
            ...
            public IPAddressVO doInTransaction(TransactionStatus status) throws InsufficientAddressCapacityException {
                ...
                // If owner has dedicated Public IP ranges, fetch IP
                // from the dedicated range
                ...
                // for direct network take ip addresses only from the
                // vlans belonging to the network
                ...
                // If all the dedicated IPs of the owner are in use
                // fetch an IP from the system pool
                if (addrs.size() == 0 && fetchFromDedicatedRange) {
                    // Verify if account is allowed to acquire IPs from
                    // the system
                ...
                if (addrs.size() == 0) {
                    if (podId != null) {
                        InsufficientAddressCapacityException ex = new InsufficientAddressCapacityException("Insufficient address capacity", Pod.class, podId);
                        // for now, we hardcode the table names, but we
                        // should ideally do a lookup for the tablename
                        // from the VO object.
                        ex.addProxyObject(ApiDBUtils.findPodById(podId).getUuid());
                        throw ex;
                    }
                ...
                if (!fetchFromDedicatedRange) {
                    // Check that the maximum number of public IPs for
                    // the given accountId will not be exceeded
                    try {
        ...
        try {
            ip = Transaction.execute(new TransactionCallbackWithException<PublicIp, InsufficientAddressCapacityException>() {
                ...
                public PublicIp doInTransaction(TransactionStatus status) throws InsufficientAddressCapacityException {
                    ...
                    if (owner == null) {
                        // this ownerId comes from owner or type
                        // Account. See the class "AccountVO" and the
                        // annotations in that class
    ...
    public boolean applyIpAssociations(Network network, boolean continueOnError) throws ResourceUnavailableException {
        ...
        // CloudStack will take a lazy approach to associate an acquired public
        // IP to a network service provider as
        // it will not know what service an acquired IP will be used for. An IP
        // is actually associated with a provider when first
        // rule is applied. Similarly when last rule on the acquired IP is
        // revoked, IP is not associated with any provider
        // but still be associated with the account. At this point just mark IP
        // as allocated or released.
        for (IPAddressVO addr : userIps) {
            if (addr.getState() == IpAddress.State.Allocating) {
                addr.setAssociatedWithNetworkId(network.getId());
                markPublicIpAsAllocated(addr);
            } else if (addr.getState() == IpAddress.State.Releasing) {
                // Cleanup all the resources for ip address if there are any,
                // and only then un-assign ip in the system
    ...
    // CloudStack will take a lazy approach to associate an acquired public IP
    // to a network service provider as
    // it will not know what a acquired IP will be used for. An IP is actually
    // associated with a provider when first
    // rule is applied. Similarly when last rule on the acquired IP is revoked,
    // IP is not associated with any provider
    // but still be associated with the account. Its up to caller of this
    // function to decide when to invoke IPAssociation
    @Override
        ...
        try {
            ...
            ipaddr = Transaction.execute(new TransactionCallbackWithException<IPAddressVO, InsufficientAddressCapacityException>() {
                ...
                public IPAddressVO doInTransaction(TransactionStatus status) throws InsufficientAddressCapacityException {
                    ...
                    // To make portable IP available as a zone level
                    // resource we need to emulate portable IP's (which
                    // are
                    // provisioned at region level) as public IP
                    // provisioned in a zone. user_ip_address and vlan
                    // combo give the
                    // identity of a public IP in zone. Create entry for
                    // portable ip in these tables.
                    ...
                    // provision the portable IP in to user_ip_address
                    // table
    ...
    public void transferPortableIP(final long ipAddrId, long currentNetworkId, long newNetworkId) throws ResourceAllocationException, ResourceUnavailableException,
        ...
        // If portable IP need to be transferred across the zones, then mark the
        // entry corresponding to portable ip
        // in user_ip_address and vlan tables so as to emulate portable IP as
        // provisioned in destination data center
        ...
        // trigger an action event for the transfer of portable IP across the
        // networks, so that external entities
        // monitoring for this event can initiate the route advertisement for
        // the availability of IP from the zoe
        ...
        try {
            pair = Transaction.execute(new TransactionCallbackWithException<Ternary<Boolean, List<NetworkOfferingVO>, Network>, Exception>() {
                ...
                public Ternary<Boolean, List<NetworkOfferingVO>, Network> doInTransaction(TransactionStatus status) throws InsufficientCapacityException,
                    ...
                    // create new Virtual network (Isolated with
                    // SourceNAT) for the user if it doesn't exist
                    ...
                    // Check if there is a source nat ip address for
                    // this account; if not - we have to allocate one
                    ...
                    // update all ips with a network id, mark them as
                    // allocated and update resourceCount/usage
    ...
    public boolean applyStaticNats(List<? extends StaticNat> staticNats, boolean continueOnError, boolean forRevoke) throws ResourceUnavailableException {
        ...
        // static NAT rules can not programmed unless IP is associated with
        // source NAT service provider, so run IP
        // association for the network so as to ensure IP is associated before
        // applying rules
        ...
        // For revoked static nat IP, set the vm_id to null, indicate it should
        // be revoked
        ...
        // if the static NAT rules configured on public IP is revoked then,
        // dis-associate IP with static NAT service provider
    ...
    // checks if there are any public IP assigned to network, that are marked
    // for one-to-one NAT that
    ...
    public void allocateDirectIp(final NicProfile nic, final DataCenter dc, final VirtualMachineProfile vm, final Network network, final String requestedIpv4,
        ...
        Transaction.execute(new TransactionCallbackWithExceptionNoReturn<InsufficientAddressCapacityException>() {
            ...
            public void doInTransactionWithoutResult(TransactionStatus status) throws InsufficientAddressCapacityException {
                // This method allocates direct ip for the Shared
                // network in Advance zones
                boolean ipv4 = false;
                ...
                // FIXME - get ipv6 address from the placeholder if it's
                // stored there
