...
public class RulesManagerImpl extends ManagerBase implements RulesManager, RulesService {
    ...
    public void checkRuleAndUserVm(FirewallRule rule, UserVm userVm, Account caller) {
        ...
        // This same owner check is actually not needed, since multiple entities
        // OperateEntry trick guarantee that
        ...
        try {
            ...
            return Transaction.execute(new TransactionCallbackWithException<PortForwardingRuleVO, NetworkRuleConflictException>() {
                ...
                public PortForwardingRuleVO doInTransaction(TransactionStatus status) throws NetworkRuleConflictException {
                    PortForwardingRuleVO newRule = new PortForwardingRuleVO(rule.getXid(), rule.getSourceIpAddressId(), rule.getSourcePortStart(), rule.getSourcePortEnd(),
                            dstIpFinal, rule.getDestinationPortStart(), rule.getDestinationPortEnd(), rule.getProtocol().toLowerCase(), networkId, accountId, domainId, vmId);
                    ...
                    try {
                        ...
                        if (newRule != null) {
                            // no need to apply the rule as it wasn't
                            // programmed on the backend yet
        ...
        try {
            ...
            if (!isSystemVm) {
                ...
                if (ipAddress.getAssociatedWithNetworkId() == null) {
                    ...
                    if (assignToVpcNtwk) {
                        ...
                        try {
                            ...
                            // associate portable IP to vpc, if network is part
                            // of VPC
                    ...
                    if (ipAddress.isPortable()) {
                        // check if destination network has StaticNat service
                        // enabled
                        _networkModel.checkIpForService(ipAddress, Service.StaticNat, networkId);
                ...
                if (ipAddress.getSystem()) {
                    // when system is enabling static NAT on system IP's (for
                    // EIP) ignore VM state
            ...
            if (_ipAddressDao.update(ipAddress.getId(), ipAddress)) {
                ...
                if (applyStaticNatForIp(ipId, false, caller, false)) {
                    performedIpAssoc = false; // ignor unassignIPFromVpcNetwork
                    // in finally block
                    return true;
                } else {
    ...
    protected void isIpReadyForStaticNat(long vmId, IPAddressVO ipAddress, String vmIp, Account caller, long callerUserId) throws NetworkRuleConflictException,
        ...
        if (!ipAddress.isOneToOneNat()) { // Dont allow to enable static nat if
            // PF/LB rules exist for the IP
        ...
        if (oldIP != null) {
            // If elasticIP functionality is supported in the network, we always
            // have to disable static nat on the old
            ...
            // If there is public ip address already associated with the vm,
            // throw an exception
    ...
    public boolean revokePortForwardingRulesForVm(long vmId) {
        ...
        for (PortForwardingRuleVO rule : rules) {
            // Mark port forwarding rule as Revoked, but don't revoke it yet
            // (apply=false)
    ...
    public boolean revokeAllPFStaticNatRulesForNetwork(long networkId, long userId, Account caller) throws ResourceUnavailableException {
        ...
        // Mark all pf rules (Active and non-Active) to be revoked, but don't
        // revoke it yet - pass apply=false
        for (PortForwardingRuleVO rule : pfRules) {
            revokePortForwardingRuleInternal(rule.getId(), caller, userId, false);
        }

        // Mark all static nat rules (Active and non-Active) to be revoked, but
        // don't revoke it yet - pass apply=false
        for (FirewallRuleVO rule : staticNatRules) {
            revokeStaticNatRuleInternal(rule.getId(), caller, userId, false);
        }
        ...
        Transaction.execute(new TransactionCallbackWithExceptionNoReturn<NetworkRuleConflictException>() {
            ...
            public void doInTransactionWithoutResult(TransactionStatus status) throws NetworkRuleConflictException {
                for (int i = 0; i < ports.length; i++) {
                    ...
                    rules[i] = new FirewallRuleVO(null, ip.getId(), ports[i], protocol, ip.getAssociatedWithNetworkId(), ip.getAllocatedToAccountId(), ip.getAllocatedInDomainId(),
    ...
    public void getSystemIpAndEnableStaticNatForVm(VirtualMachine vm, boolean getNewIp) throws InsufficientAddressCapacityException {
        ...
        for (Nic nic : nics) {
            ...
            if (offering.getElasticIp()) {
                ...
                // for user VM's associate public IP only if offering is marked
                // to associate a public IP by default on start of VM
    ...
    public List<FirewallRuleVO> listAssociatedRulesForGuestNic(Nic nic) {
        ...
        for (IpAddress ip : staticNatIps) {
            if (ip.getVmIp() != null && ip.getVmIp().equals(nic.getIp4Address())) {
                ...
                // generate a static Nat rule on the fly because staticNATrule
                // does not persist into db anymore
                // FIX ME
                FirewallRuleVO staticNatRule = new FirewallRuleVO(null, ip.getId(), 0, 65535, NetUtils.ALL_PROTO.toString(), nic.getNetworkId(), vm.getAccountId(),
                        vm.getDomainId(), Purpose.StaticNat, null, null, null, null, null);
