...
/**
 * This class takes the input stream and turns it multiple streams. DIME version
 * 0 format
 * 
 * <pre>
 *  0                   1                   2                   3
 ...
 *
 * Incoming data is DIME encoded when its MIME type is "application/dime". Then
 * use this class to pull out 2 streams: (1) The first stream is the SOAP
 * request, (2) The second stream is a chunked attachment (e.g., a file to
 * store)
 *
...
public class DimeDelimitedInputStream extends FilterInputStream {
     ...
     * stream can be a SOAP message, and the second stream a binary attachment
     * (e.g., a file). During reading after an EOF is returned, this function
     * should be called to see if there is another stream following the last.
     *
     ...
     *
     * @param is
     *            the <code>InputStream</code> to wrap
     * @throws IOException
    ...
    /**
     * Make sure to skip the pad which appear in several parts of a DIME
     * message.
     * 
     * @param size
     * @return
    ...
    protected int _read(byte[] b, int off, int len) throws IOException {
        ...
        if (0 > bytes2read) {
            ...
            // in theory the last chunk of data should also have been padded,
            // but lets be tolerant of that.
            dataPadLength = 0;
            ...
            try {
                ...
                // in theory the last chunk of data should also have been
                // padded, but lets be tolerant of that.
                if (!ME)
                    throw e;
                else {
                    dataPadLength = 0;
    ...
    /**
     * The DIME header is read into local class data fields and are not passed
     * as part of the stream data.
     *
     * @param isChunk
     * @throws IOException
     */
    protected void readHeader(boolean isChunk) throws IOException {
        ...
        if (isChunk) {
            ...
            dataPadLength -= readPad(dataPadLength); // Just in case it was left
                                                     // over.
        }
        ...
        if (0 != optionsLength) {
            ...
            optBytes = null; // throw it away, don't know anything about
                             // options.
     ...
     *
     * @param b
     *            is the array to read into. Read as much as possible into the
     *            size of this array.
     * @return the number of bytes read. -1 if endof stream
     * @throws IOException
     *             if data could not be read from the stream
    ...
    /**
     * Closes the stream. This will take care of flushing any remaining data to
     * the stream. Multiple calls to this method will result in the stream being
     * closed once and then all subsequent calls being ignored.
     *
     * @throws IOException
    ...
    public void close() throws IOException {
        ...
        if (bytesRead < recordLength || moreChunks) {
            // We need get this off the stream. Easy way to flush through the
            // stream;
            byte[] readrest = new byte[1024 * 16];
            int bread = 0;

            do {
                bread = _read(readrest, 0, readrest.length); // should also
                                                             // close the
                                                             // original stream.
            } while (bread > -1);
        }
    ...
    /**
     * Skip n bytes of data in the DIME stream, while reading and processing any
     * headers in the current stream.
     *
     * @param n
