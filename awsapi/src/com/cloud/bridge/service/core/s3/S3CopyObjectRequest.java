...
public class S3CopyObjectRequest extends S3Request {
    ...
    public void setDataDirective(String dataDirective) {
        if (null == dataDirective)
            return;

        if (dataDirective.equalsIgnoreCase("COPY"))
            directive = MetadataDirective.COPY;
        else if (dataDirective.equalsIgnoreCase("REPLACE"))
            directive = MetadataDirective.REPLACE;
        else
            throw new UnsupportedOperationException("Unknown Metadata Directive: " + dataDirective);
    }
