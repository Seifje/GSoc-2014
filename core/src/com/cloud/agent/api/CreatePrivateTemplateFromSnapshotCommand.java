...
public class CreatePrivateTemplateFromSnapshotCommand extends SnapshotCommand {
    ...
    /**
     * Given the UUID of a backed up snapshot VHD file on the secondary storage,
     * the execute of this command does 1) Get the parent chain of this VHD all
     * the way up to the root, say VHDList 2) Copy all the files in the VHDlist
     * to some temp location 3) Coalesce all the VHDs to one VHD which contains
     * all the data of the volume. This invokes the DeletePreviousBackupCommand
     * for each VHD 4) Rename the UUID of this VHD
     *
     ...
     *            get it, you are welcome. It may not be the UUID of the base
     *            copy of the snapshot, if no data was written since last
     *            snapshot.
     * @param origTemplateInstallPath
     *            The install path of the original template VHD on the secondary
     */
