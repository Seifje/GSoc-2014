...
public class HAProxyConfigurator implements LoadBalancerConfigurator {
    ...
    private List<String> getRulesForPool(String poolName, List<PortForwardingRuleTO> fwRules) {
        ...
        for (PortForwardingRuleTO rule : fwRules) {
            ...
            sb = new StringBuilder();
            sb.append("\t").append("server ").append(poolName).append("_").append(Integer.toString(i++)).append(" ").append(rule.getDstIp()).append(":")
     ...
     * postonly ] [ domain <domain> ]* Enable cookie-based persistence in a
     * backend. May be used in sections : defaults | frontend | listen | backend
     * yes | no | yes | yes Arguments : <name> is the name of the cookie which
     * will be monitored, modified or inserted in order to bring persistence.
     * This cookie is sent to the client via a "Set-Cookie" header in the
     * response, and is brought back by the client in a "Cookie" header in all
     * requests. Special care should be taken to choose a name which does not
     * conflict with any likely application cookie. Also, if the same backends
     * are subject to be used by the same clients (eg: HTTP/HTTPS), care should
     * be taken to use different cookie names between all backends if
     * persistence between them is not desired.
     *
     * rewrite This keyword indicates that the cookie will be provided by the
     * server and that haproxy will have to modify its value to set the server's
     * identifier in it. This mode is handy when the management of complex
     * combinations of "Set-cookie" and "Cache-control" headers is left to the
     * application. The application can then decide whether or not it is
     * appropriate to emit a persistence cookie. Since all responses should be
     * monitored, this mode only works in HTTP close mode. Unless the
     * application behaviour is very complex and/or broken, it is advised not to
     * start with this mode for new deployments. This keyword is incompatible
     * with "insert" and "prefix".
     *
     * insert This keyword indicates that the persistence cookie will have to be
     * inserted by haproxy in the responses. If the server emits a cookie with
     * the same name, it will be replaced anyway. For this reason, this mode can
     * be used to upgrade existing configurations running in the "rewrite" mode.
     * The cookie will only be a session cookie and will not be stored on the
     * client's disk. Due to caching effects, it is generally wise to add the
     * "indirect" and "nocache" or "postonly" keywords (see below). The "insert"
     * keyword is not compatible with "rewrite" and "prefix".
     *
     * prefix This keyword indicates that instead of relying on a dedicated
     * cookie for the persistence, an existing one will be completed. This may
     * be needed in some specific environments where the client does not support
     * more than one single cookie and the application already needs it. In this
     * case, whenever the server sets a cookie named <name>, it will be prefixed
     * with the server's identifier and a delimiter. The prefix will be removed
     * from all client requests so that the server still finds the cookie it
     * emitted. Since all requests and responses are subject to being modified,
     * this mode requires the HTTP close mode. The "prefix" keyword is not
     * compatible with "rewrite" and "insert".
     *
     * indirect When this option is specified in insert mode, cookies will only
     * be added when the server was not reached after a direct access, which
     * means that only when a server is elected after applying a load-balancing
     * algorithm, or after a redispatch, then the cookie will be inserted. If
     * the client has all the required information to connect to the same server
     * next time, no further cookie will be inserted. In all cases, when the
     * "indirect" option is used in insert mode, the cookie is always removed
     * from the requests transmitted to the server. The persistence mechanism
     * then becomes totally transparent from the application point of view.
     *
     * nocache This option is recommended in conjunction with the insert mode
     * when there is a cache between the client and HAProxy, as it ensures that
     * a cacheable response will be tagged non-cacheable if a cookie needs to be
     * inserted. This is important because if all persistence cookies are added
     * on a cacheable home page for instance, then all customers will then fetch
     * the page from an outer cache and will all share the same persistence
     * cookie, leading to one server receiving much more traffic than others.
     * See also the "insert" and "postonly" options.
     *
     * postonly This option ensures that cookie insertion will only be performed
     * on responses to POST requests. It is an alternative to the "nocache"
     * option, because POST responses are not cacheable, so this ensures that
     * the persistence cookie will never get cached. Since most sites do not
     * need any sort of persistence before the first POST which generally is a
     * login request, this is a very efficient method to optimize caching
     * without risking to find a persistence cookie in the cache. See also the
     * "insert" and "nocache" options.
     *
     * domain This option allows to specify the domain at which a cookie is
     * inserted. It requires exactly one parameter: a valid domain name. If the
     * domain begins with a dot, the browser is allowed to use it for any host
     * ending with that name. It is also possible to specify several domain
     * names by invoking this option multiple times. Some browsers might have
     * small limits on the number of domains, so be careful when doing that. For
     * the record, sending 10 domains to MSIE 6 or Firefox 2 works as expected.
     *
     * There can be only one persistence cookie per HTTP backend, and it can be
     * declared in a defaults section. The value of the cookie will be the value
     * indicated after the "cookie" keyword in a "server" statement. If no
     * cookie is declared for a given server, the cookie is not set.
     *
     * Examples : cookie JSESSIONID prefix cookie SRV insert indirect nocache
     * cookie SRV insert postonly indirect
     *
     *
     * appsession <cookie> len <length> timeout <holdtime> [request-learn]
     * [prefix] [mode <path-parameters|query-string>] Define session stickiness
     * on an existing application cookie. May be used in sections : defaults |
     * frontend | listen | backend no | no | yes | yes Arguments : <cookie> this
     * is the name of the cookie used by the application and which HAProxy will
     * have to learn for each new session.
     *
     * <length> this is the max number of characters that will be memorized and
     * checked in each cookie value.
     *
     * <holdtime> this is the time after which the cookie will be removed from
     * memory if unused. If no unit is specified, this time is in milliseconds.
     *
     * request-learn If this option is specified, then haproxy will be able to
     * learn the cookie found in the request in case the server does not specify
     * any in response. This is typically what happens with PHPSESSID cookies,
     * or when haproxy's session expires before the application's session and
     * the correct server is selected. It is recommended to specify this option
     * to improve reliability.
     *
     * prefix When this option is specified, haproxy will match on the cookie
     * prefix (or URL parameter prefix). The appsession value is the data
     * following this prefix.
     *
     * Example : appsession ASPSESSIONID len 64 timeout 3h prefix
     *
     * This will match the cookie ASPSESSIONIDXXXX=XXXXX, the appsession value
     * will be XXXX=XXXXX.
     *
     * mode This option allows to change the URL parser mode. 2 modes are
     * currently supported : - path-parameters : The parser looks for the
     * appsession in the path parameters part (each parameter is separated by a
     * semi-colon), which is convenient for JSESSIONID for example. This is the
     * default mode if the option is not set. - query-string : In this mode, the
     * parser will look for the appsession in the query string.
     *
     * When an application cookie is defined in a backend, HAProxy will check
     * when the server sets such a cookie, and will store its value in a table,
     * and associate it with the server's identifier. Up to <length> characters
     * from the value will be retained. On each connection, haproxy will look
     * for this cookie both in the "Cookie:" headers, and as a URL parameter
     * (depending on the mode used). If a known value is found, the client will
     * be directed to the server associated with this value. Otherwise, the load
     * balancing algorithm is applied. Cookies are automatically removed from
     * memory when they have been unused for a duration longer than <holdtime>.
     *
    ...
    private String getLbSubRuleForStickiness(LoadBalancerTO lbTO) {
        ...
        for (StickinessPolicyTO stickinessPolicy : lbTO.getStickinessPolicies()) {
             ...
             * nocache ] [ postonly ] [ domain <domain> ]*
             */
            if (StickinessMethodType.LBCookieBased.getName().equalsIgnoreCase(stickinessPolicy.getMethodName())) {
                ...
                /*
                 * Error is silently swallowed. Not supposed to reach here,
                 * validation of methods are done at the higher layer
                 */
    ...
    private List<String> getRulesForPool(LoadBalancerTO lbTO, boolean keepAliveEnabled) {
        ...
        for (DestinationTO dest : lbTO.getDestinations()) {
            ...
            sb = new StringBuilder();
            sb.append("\t").append("server ").append(poolName).append("_").append(Integer.toString(i++)).append(" ").append(dest.getDestIp()).append(":")
    ...
    public String[] generateConfiguration(LoadBalancerConfigCommand lbCmd) {
        ...
        // note that this is overwritten on the String in the static
        // ArrayList<String>
