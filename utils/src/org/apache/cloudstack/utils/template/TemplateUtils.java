...
public class TemplateUtils {
    ...
    // given a path, returns empty if path is supported image, and the file type
    // if unsupported
    // this is meant to catch things like accidental upload of ASCII text .vmdk
    // descriptor
    public static String checkTemplateFormat(String path, String url) {
        // note 'path' was generated by us so it should be safe on the cmdline,
        // be wary of 'url'
        String command = "file ";
    ...
    public static boolean isCorrectExtension(String url, String ext) {
        if (url.toLowerCase().endsWith(ext) || url.toLowerCase().endsWith(ext + ".gz") || url.toLowerCase().endsWith(ext + ".bz2") || url.toLowerCase().endsWith(ext + ".zip")) {
            return true;
        }
        return false;
    }
    ...
    public static boolean isCompressedExtension(String path) {
        if (path.toLowerCase().endsWith(".gz") || path.toLowerCase().endsWith(".bz2") || path.toLowerCase().endsWith(".zip")) {
            return true;
        }
        return false;
    }
