...
public class ServerPaletteUpdate extends BaseElement {
    ...
    public void handleData(ByteBuffer buf, Link link) {
        ...
        // (4 bytes): A 32-bit, unsigned integer. The number of RGB triplets in
        // the
        // paletteData field. This field MUST be set to 256 (the number of
        // entries
        // in an 8 bpp palette).
        int numberColors = (int)buf.readUnsignedIntLE();
        if (numberColors != 256)
            throw new RuntimeException("Unexpected value for number of color field in server Palette Update packet. Expected value: 256 colors, actual value: " + numberColors
                    + ", data: " + buf + ".");

        // (variable): An array of palette entries in RGB triplet format packed
        // on
