...
public class ClientNtlmsspNegotiate extends OneTimeSwitch {
    ...
    public NegoFlags clientConfigFlags = new NegoFlags().set_NEGOTIATE_56().set_NEGOTIATE_KEY_EXCH().set_NEGOTIATE_128().set_NEGOTIATE_VERSION()
            .set_NEGOTIATE_EXTENDED_SESSION_SECURITY().set_NEGOTIATE_ALWAYS_SIGN().set_NEGOTIATE_NTLM().set_NEGOTIATE_LM_KEY().set_NEGOTIATE_SEAL().set_NEGOTIATE_SIGN()
            .set_REQUEST_TARGET().set_NEGOTIATE_OEM().set_NEGOTIATE_UNICODE();
    ...
    protected void onStart() {
        ...
        ntlmState.negotiateMessage = negoToken.toByteArray(); // Store message
        // for MIC
        // calculation in
        // AUTH message
    ...
    private ByteBuffer generateNegotiateMessage() {
        ...
        // application,
        // the Version field MUST be set to the current version (section
        // 2.2.2.10),
    ...
    public static void main(String args[]) {
        ...
        byte[] packet = new byte[] {
                ...
                (byte) 0xa0,
                (byte) 0x03, // TAG: [0] (constructed) LEN: 3 byte
                (byte) 0x02,
                (byte) 0x01,
                ...
                (byte) 0xa1,
                (byte) 0x30, // TAG: [1] (constructed) LEN: 48 bytes
                (byte) 0x30,
                (byte) 0x2e, // TAG: [UNIVERSAL 16] (constructed) "SEQUENCE"
                // LEN: 46 bytes
                (byte) 0x30,
                (byte) 0x2c, // TAG: [UNIVERSAL 16] (constructed) "SEQUENCE"
                // LEN: 44 bytes
                (byte) 0xa0,
                (byte) 0x2a, // TAG: [0] (constructed) LEN: 42 bytes

                (byte) 0x04,
                (byte) 0x28, // TAG: [UNIVERSAL 4] (primitive) "OCTET STRING"
                // LEN: 40 bytes
                ...
                (byte) 0x82,
                (byte) 0x08,
                (byte) 0xe2, // Flags: 0xe20882b7 (LE)

                (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00,
                (byte) 0x00, (byte) 0x00,
                (byte) 0x00,
                (byte) 0x00, // Domain (security buffer, 8bit, 8 bytes): length:
                // 0x0000 (LE), allocated space: 0x0000 (LE),
                // offset: 0x00000000 (LE)
                (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00,
                (byte) 0x00, (byte) 0x00, (byte) 0x00,
                (byte) 0x00, // Workstation (security buffer, 8bit, 8 bytes):
                // length: 0x0000 (LE), allocated space: 0x0000
                // (LE), offset: 0x00000000 (LE)
                (byte) 0x06, (byte) 0x01, (byte) 0xb1, (byte) 0x1d,
                (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x0f, // OS
                // Version:
                // 6.1
                // (Build
                // 7601);
