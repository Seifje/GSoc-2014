...
public class CapacityDaoImpl extends GenericDaoBase<CapacityVO, Long> implements CapacityDao {
    ...
    /*
     * In the below query"LIST_CLUSTERS_CROSSING_THRESHOLD" the threshold value
     * is getting from the cluster_details table if not present then it gets
     * from the global configuration
     *
     * CASE statement works like if (cluster_details table has threshold value)
     * then if (value from the cluster_details table is not null) then query
     * from the cluster_details table else query from the configuration table
     * else query from the configuration table
     */
    ...
    /*
     * public static String preparePlaceHolders(int length) { StringBuilder
     * builder = new StringBuilder(); for (int i = 0; i < length;) {
     * builder.append("?"); if (++i < length) { builder.append(","); } } return
     * builder.toString(); }
     *
     * public static void setValues(PreparedStatement preparedStatement,
     * Object... values) throws SQLException { for (int i = 0; i <
     * values.length; i++) { preparedStatement.setObject(i + 1, values[i]); } }
     */
