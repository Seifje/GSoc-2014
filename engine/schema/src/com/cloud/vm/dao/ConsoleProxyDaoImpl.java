...
public class ConsoleProxyDaoImpl extends GenericDaoBase<ConsoleProxyVO, Long> implements ConsoleProxyDao {
    ...
    private static final String PROXY_ASSIGNMENT_MATRIX = "SELECT c.id, count(runningVm.id) AS count "
            ...
            + "  (v.state='Running' OR v.state='Creating' OR v.state='Starting' OR v.state='Migrating')) " + " AS runningVm ON c.id = runningVm.proxy_id WHERE i.state='Running' "
            + " GROUP BY c.id";
    ...
    //
    private static final String DATACENTER_PROXY_MATRIX = "SELECT d.id, d.name, count(dcid) as count" + " FROM data_center as d" + " LEFT JOIN ("
