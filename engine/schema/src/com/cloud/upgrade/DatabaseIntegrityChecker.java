...
public class DatabaseIntegrityChecker extends AdapterBase implements SystemIntegrityChecker {
    ...
    /*
     * Check if there were multiple hosts connect to the same local storage.
     * This is from a 2.1.x bug, we didn't prevent adding host with the same IP.
     */
        ...
        try {
            ...
            try {
                ...
                PreparedStatement pstmt = conn
                        .prepareStatement("SELECT pool_id FROM host INNER JOIN storage_pool_host_ref INNER JOIN storage_pool WHERE storage_pool.id = storage_pool_host_ref.pool_id and storage_pool.pool_type='LVM' AND host.id=storage_pool_host_ref.host_id AND host.removed IS NULL group by pool_id having count(*) > 1");
                ...
                while (rs.next()) {
                    ...
                    pstmt = conn
                            .prepareStatement("select id, status, removed, private_ip_address from host where id in (select host_id from storage_pool_host_ref where pool_id=?)");
