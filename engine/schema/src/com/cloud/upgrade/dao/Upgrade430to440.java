...
public class Upgrade430to440 implements DbUpgrade {
    ...
    private void addExtractTemplateAndVolumeColumns(Connection conn) {
        ...
        try (PreparedStatement selectTemplateInfostmt = conn
                .prepareStatement("SELECT *  FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'cloud' AND TABLE_NAME = 'template_store_ref' AND COLUMN_NAME = 'download_url_created'");
                ResultSet templateInfoResults = selectTemplateInfostmt.executeQuery();
                PreparedStatement addDownloadUrlCreatedToTemplateStorerefstatement = conn
                        .prepareStatement("ALTER TABLE `cloud`.`template_store_ref` ADD COLUMN `download_url_created` datetime");
                PreparedStatement addDownloadUrlToTemplateStorerefstatement = conn
                        .prepareStatement("ALTER TABLE `cloud`.`template_store_ref` ADD COLUMN `download_url` varchar(255)");
                PreparedStatement selectVolumeInfostmt = conn
                        .prepareStatement("SELECT *  FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = 'cloud' AND TABLE_NAME = 'volume_store_ref' AND COLUMN_NAME = 'download_url_created'");
                ResultSet volumeInfoResults = selectVolumeInfostmt.executeQuery();
                PreparedStatement addDownloadUrlCreatedToVolumeStorerefstatement = conn
                        .prepareStatement("ALTER TABLE `cloud`.`volume_store_ref` ADD COLUMN `download_url_created` datetime");) {
            ...
            // Add download_url_created to volume_store_ref - note download_url
            // already exists
    ...
    private void secondaryIpsAccountAndDomainIdsUpdate(Connection conn) {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(secondIpsSql); ResultSet rs1 = pstmt.executeQuery();) {
            while (rs1.next()) {
                    ...
                    try (ResultSet vmRs = pstmtVm.executeQuery();) {
                        ...
                        if (vmRs.next()) {
                            ...
                            if (vmAccountId != accountId && vmAccountId != domainId) {
                                // update the secondary ip accountid and
                                // domainid to vm accountid domainid
                                // check the network type. If network is shared
                                // accountid doaminid needs to be updated in
                                // in both nic_secondary_ips table and
                                    ...
                                    try (ResultSet networkRs = pstmtNw.executeQuery();) {
                                        if (networkRs.next()) {
                                            ...
                                            if (guesttype.equals(Network.GuestType.Shared.toString())) {
                                                try (PreparedStatement pstmtUpdate = conn
                                                        .prepareStatement("UPDATE `cloud`.`user_ip_address` SET account_id = ?, domain_id= ? WHERE public_ip_address = ?");) {
                                                    pstmtUpdate.setLong(1, vmAccountId);
                                                    pstmtUpdate.setLong(2, vmDomainId);
