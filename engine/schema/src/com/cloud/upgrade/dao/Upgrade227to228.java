...
public class Upgrade227to228 implements DbUpgrade {
    ...
    // this method inserts missing volume.delete events (events were missing
    // when vm failed to create)
    private void updateVolumeUsageRecords(Connection conn) {
        try {
            ...
            PreparedStatement pstmt = conn
                    .prepareStatement("select id, account_id, data_center_id, name from volumes where state='Destroy' and id in (select resource_id from usage_event where type='volume.create') and id not in (select resource_id from usage_event where type='volume.delete')");
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                ...
                pstmt = conn
                        .prepareStatement("insert into usage_event (type, account_id, created, zone_id, resource_name, resource_id) values ('VOLUME.DELETE', ?, now(), ?, ?, ?)");
