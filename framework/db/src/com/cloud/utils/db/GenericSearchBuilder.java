...
/**
 * GenericSearchBuilder is used to build a search based on a VO object. It can
 * select the result into a native type, the entity object, or a composite
 * object depending on what's needed.
 *
 * The way to use GenericSearchBuilder is to use it to build a search at load
 * time so it should be declared at class constructions. It allows queries to be
 * constructed completely in Java and parameters have String tokens that can be
 * replaced during runtime with SearchCriteria. Because GenericSearchBuilder is
 * created at load time and SearchCriteria is used at runtime, the search query
 * creation and the parameter value setting are separated in the code. While
 * that's tougher on the coder to maintain, what you gain is that all string
 * constructions are done at load time rather than runtime and, more
 * importantly, the proper construction can be checked when components are being
 * loaded. However, if you prefer to just construct the entire search at
 * runtime, you can use GenericQueryBuilder.
 *
 * <code>
 ...
 *
 * @param <T>
 *            VO object this Search is build for.
 * @param <K>
 *            Result object that should contain the results.
 */
public class GenericSearchBuilder<T, K> extends SearchBase<GenericSearchBuilder<T, K>, T, K> {
     ...
     *
     * @param name
     *            param name you will use later to set the values in this search
     *            condition.
     * @param field
     *            SearchBuilder.entity().get*() which refers to the field that
     *            you're searching on.
     * @param op
    ...
    /**
     * Adds an AND condition. Some prefer this method because it looks like the
     * actual SQL query.
     *
     * @param field
     *            field of entity object
     * @param op
     *            operator of the search condition
     * @param name
     *            param name used to later to set parameter value
     * @return this
     */
    ...
    /**
     * Adds an AND condition but allows for a preset value to be set for this
     * conditio.
     *
     * @param field
     ...
     *            operator
     * @param name
     *            param name to refer to the value later.
     * @return this
     */
     ...
     *
     * @param name
     *            parameter name used to set the parameter value later.
     * @param field
     *            field of the entity object
     * @param op
     *            operator
    ...
    /**
     * Convenience method to create the search criteria and set a parameter in
     * the search.
     *
     * @param name
     *            parameter name set during construction
     * @param values
     *            values to be inserted for that parameter
