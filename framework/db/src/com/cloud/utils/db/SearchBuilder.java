 ...
 *
 * 1. SearchBuilder is declared as final because it should not change after load
 * time. 2. search.entity().getHostId() allows you to declare which field you
 * are searching on. By doing this, we take advantage of the compiler to check
 * that we are not searching on obsolete fields. You should try to use this as
 * much as possible as oppose to the addAndByField() and addOrByField() methods.
 * 3. Note that the same SearchBuilder is used to create multiple
 * SearchCriteria. This is equivalent to clearing the parameters on
 * PreparedStatement. The multiple SearchCriteria does not interfere with each
 * other. 4. Note that the same field (getHostId()) was specified with two param
 * names. This is basically allowing you to reuse the same field in two parts of
 * the search.
 *
 * {@code
