...
public interface AsyncJobManager extends Manager {
    ...
    /**
     * A running thread inside management server can have a 1:1 linked pseudo
     * job. This is to help make some legacy code work without too dramatic
     * changes.
     *
     ...
     * (usually VmWork jobs) in synchronous way. Caller needs to use
     * waitAndCheck() to check the completion status of the down-level job
     *
     * Due to the amount of legacy code that relies on synchronous-call
     * semantics, this form of joinJob is used mostly
     *
     *
     * @param jobId
     *            upper job that is going to wait the completion of a down-level
     *            job
     * @param joinJobId
     *            down-level job
     */
    ...
    /**
     * Used by upper level job to wait for completion of a down-level job
     * (usually VmWork jobs) in asynchronous way, it will cause upper job to
     * cease current execution, upper job will be rescheduled to execute
     * periodically or on wakeup events detected from message bus
     *
     * @param jobId
     *            upper job that is going to wait the completion of a down-level
     *            job
     ...
     * @param joinStatus
     *            AsyncJobConstants status code to indicate success or failure
     *            of the down-level job
     * @param joinResult
     *            object-stream serialized result object this is primarily used
     *            by down-level job to pass error exception objects for legacy
     *            code to work. To help pass exception object easier, we use
     ...
     * fully-asynchronous mode that uses async-feature of joinJob/disjoinJob
     *
     * @param wakupTopicsOnMessageBus
     *            topic on message bus to wakeup the wait
     * @param checkIntervalInMilliSeconds
     *            time to break out wait for checking predicate condition
     * @param timeoutInMiliseconds
     *            time out to break out the whole wait process
     * @param predicate
     * @return true, predicate condition is satisfied false, wait is timed out
     */
